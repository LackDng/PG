...
def vi(text):
    ...
    Vi du: 'Nguyen My Chau' <- 'Nguyễn Mỹ Châu', 'Da Nang' <- 'Đà Nẵng'
    """
...
def _print_items_section(p, W, session, ref_time):
    ...
    COL_AMT = 10   # do rong cot so tien (ky tu)
    COL_NAME = W - COL_AMT

    p.set(bold=True)
    p.text(f"{'TEN HANG HOA':<{COL_NAME}}{'T.TIEN':>{COL_AMT}}\n")
    p.set(bold=False)
    p.text("-" * W + "\n")
    ...
    for so in service_orders:
        ...
        amt_str = f"{cost:,}d"
        p.text(f"{name:<{COL_NAME}}{amt_str:>{COL_AMT}}\n")
...
def _print_qr_section(p, W, bank_bin, bank_account, account_holder, amount, room_name):
    ...
    try:
        ...
        p.set(align="left")
        if account_holder:
            p.text(f"CTK: {vi(account_holder)}\n")
...
def _print_invoice_content(p, invoice):
    ...
    p.set(bold=True)
    p.text(f"{'Dich vu:':<{COL_NAME}}{invoice.total_service:>{COL_AMT},}d\n")
    p.text(f"{'Do an/uong:':<{COL_NAME}}{invoice.total_food:>{COL_AMT},}d\n")
    if invoice.total_outside > 0:
        p.text(f"{'Mua ngoai:':<{COL_NAME}}{invoice.total_outside:>{COL_AMT},}d\n")
    p.text(f"{'Tong cong:':<{COL_NAME}}{invoice.subtotal:>{COL_AMT},}d\n")
    if invoice.discount_percent > 0:
        label = f"Giam {invoice.discount_percent}%:"
        p.text(f"{label:<{COL_NAME}}{-invoice.discount_amount:>{COL_AMT},}d\n")
        p.text(f"{'Sau giam:':<{COL_NAME}}{invoice.total_after_discount:>{COL_AMT},}d\n")
    p.set(bold=False)
    if invoice.tip > 0:
        p.text(f"{'Tip:':<{COL_NAME}}{invoice.tip:>{COL_AMT},}d\n")

    p.text("=" * W + "\n")
    p.set(bold=True, align="center")
    p.text("TONG THANH TOAN:\n")
    p.text(f"{invoice.total_after_discount:,}d\n")
    p.set(bold=False, align="left")

    p.text("-" * W + "\n")
    if invoice.payment_method == "cash":
        p.text(f"{'Tien mat:':<{COL_NAME}}{invoice.cash_amount:>{COL_AMT},}d\n")
    elif invoice.payment_method == "transfer":
        p.text(f"{'Chuyen khoan:':<{COL_NAME}}{invoice.transfer_amount:>{COL_AMT},}d\n")
    else:
        p.text(f"{'Tien mat:':<{COL_NAME}}{invoice.cash_amount:>{COL_AMT},}d\n")
        p.text(f"{'Chuyen khoan:':<{COL_NAME}}{invoice.transfer_amount:>{COL_AMT},}d\n")

    # QR chuyen khoan — luon hien neu co cau hinh ngan hang
    ...
    if bank_bin and bank_account:
        if invoice.payment_method == "mixed":
            qr_amount = invoice.transfer_amount
        else:
            qr_amount = invoice.total_after_discount
    ...
    if invoice.note:
        p.text("=" * W + "\n")
        p.set(align="left")
        p.text(f"Ghi chu: {vi(invoice.note)}\n")
...
def _print_check_content(p, session):
    ...
    p.set(align="center")
    p.text(f"Phong: {vi(session.room.name)}\n")
    p.text("-" * W + "\n")
    p.set(align="left")
    ...
    p.set(bold=True)
    p.text(f"{'Dich vu:':<{COL_NAME}}{totals['total_service']:>{COL_AMT},}d\n")
    p.text(f"{'Do an/uong:':<{COL_NAME}}{totals['total_food']:>{COL_AMT},}d\n")
    if totals["total_outside"] > 0:
        p.text(f"{'Mua ngoai:':<{COL_NAME}}{totals['total_outside']:>{COL_AMT},}d\n")
    p.text("=" * W + "\n")
    p.set(align="center")
    p.text("TAM TINH:\n")
    p.text(f"{totals['subtotal']:,}d\n")
    p.set(bold=False)
    p.text("=" * W + "\n")
    p.set(align="center")
...
def _print_temp_content(p, session, discount_percent=0):
    ...
    p.text("-" * W + "\n")
    p.set(bold=True)
    p.text(f"{'Tam tinh:':<{COL_NAME}}{subtotal:>{COL_AMT},}d\n")
    if discount_percent > 0:
        label = f"Giam {discount_percent}%:"
        p.text(f"{label:<{COL_NAME}}{-discount_amount:>{COL_AMT},}d\n")
    p.text("=" * W + "\n")
    p.set(align="center")
    p.text("TONG THANH TOAN:\n")
    p.text(f"{total_after_discount:,}d\n")
    p.set(bold=False, align="left")
